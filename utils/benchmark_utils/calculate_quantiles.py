...
def calculate_quantiles_multi_df_metrics(df_metric_mapping: Dict[DataFrame, str],
                                        order_type_column: str = 'order_type',
                                        quantiles_range: tuple = (5, 95),
                                        interval: int = 5,
                                        df_names: List[str] = None) -> DataFrame:
    """
    Calculate quantiles for different metrics across multiple DataFrames, grouped by order type.
    Returns DataFrame with percentiles as columns and (dataframe_metric, order_type) as rows.
    ...
    order_type_column : str, default 'order_type'
        Name of the column containing order type values
    ...
    --------
    DataFrame
        DataFrame with rows as (dataframe_metric, order_type) and columns as percentile values
        Example columns: dataframe_metric, order_type, p5, p10, p15, ..., p95
    """
    ...
    for i, (df, metric_column) in enumerate(df_metric_mapping.items()):
        ...
        if order_type_column not in df.columns:
            print(f"Warning: Column '{order_type_column}' not found in DataFrame '{df_name}'. Skipping...")
            continue
       
        # Calculate all quantiles for this DataFrame's metric, grouped by order_type
        quantile_exprs = []
        ...
        # Group by order_type and calculate quantiles for each group
        quantile_df = df.groupBy(order_type_column).agg(*quantile_exprs)
       
        # Add the dataframe_metric column as a literal
        quantile_df = quantile_df.withColumn("dataframe_metric", lit(metric_column))
       
        # Reorder columns to put dataframe_metric and order_type first
        column_order = ["dataframe_metric", order_type_column] + [f'p{p}' for p in quantile_percentages]
    ...
    if result_dfs:
        ...
        schema_fields = [
            StructField("dataframe_metric", StringType(), True),
            StructField(order_type_column, StringType(), True)
        ]
...
def calculate_quantiles_with_cohorts_opt(df_metric_mapping: Dict[DataFrame, str],
                                   ...
                                   df_names: List[str] = None) -> DataFrame:
    """
    Calculate quantiles for different metrics across multiple DataFrames by cohort groups and order type.
    Returns DataFrame with percentiles as columns and (dataframe_metric, cohort, order_type) as rows.
   
    OPTIMIZED VERSION: Uses groupBy for both cohort and order_type simultaneously.
    This reduces N*M separate Spark jobs to 1 job per DataFrame.
    ...
    --------
    DataFrame
        DataFrame with rows as 'dataframe_metric_cohort_ordertype' and columns as percentile values
        Example rows: sales_high_Type_A, sales_medium_Type_B, cost_premium_Type_A
    """
    ...
    for i, (df, metric_column) in enumerate(df_metric_mapping.items()):
        ...
        # Check if metric column, cohort column, and order_type column exist
        if metric_column not in df.columns:
            print(f"Warning: Column '{metric_column}' not found in DataFrame '{df_name}'. Skipping...")
            continue
        ...
        if order_type_column not in df.columns:
            print(f"Warning: Order type column '{order_type_column}' not found in DataFrame '{df_name}'. Skipping...")
            continue
       
        # OPTIMIZATION: Calculate all quantiles for all cohorts and order types in ONE operation
        quantile_exprs = []
        ...
        # Single groupBy operation for all cohorts and order types
        cohort_quantiles = df.groupBy(cohort_column, order_type_column).agg(*quantile_exprs)
       
        # Add the dataframe_metric_cohort_ordertype column
        cohort_quantiles = cohort_quantiles.withColumn(
            "dataframe_metric_cohort_ordertype",
            F.concat(lit(f"{metric_column}_"), col(cohort_column), lit("_"), col(order_type_column))
        )
       
        # Select and reorder columns
        column_order = ["dataframe_metric_cohort_ordertype"] + [f'p{p}' for p in quantile_percentages]
    ...
    if result_dfs:
        ...
        # Return empty DataFrame with schema if no data
        spark = SparkSession.getActiveSession()
        schema_fields = [StructField("dataframe_metric_cohort_ordertype", StringType(), True)]
        for percentage in quantile_percentages:
            schema_fields.append(StructField(f"p{percentage}", DoubleType(), True))
