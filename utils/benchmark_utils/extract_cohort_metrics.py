...
def transform_dataframe(df: DataFrame) -> DataFrame:
    """
    Transforms dataframe by splitting the 'dataframe_metric_cohort' column 
    ...
    Args:
        df: Input DataFrame with 'dataframe_metric_cohort' column
    ...
    df_transformed = df.withColumn(
        "has_per_lb",
        col("dataframe_metric_cohort").contains("_per_lb_")
    ).withColumn(
        "has_pct_sales",
        col("dataframe_metric_cohort").contains("_pct_sales_")
    )
    ...
    df_transformed = df_transformed.withColumn(
        ...
        when(col("has_per_lb"),
             regexp_replace(
                 split(col("dataframe_metric_cohort"), "_per_lb_")[0],
                 "^(.*)$", "$1"
             )
        ).when(col("has_pct_sales"),
             regexp_replace(
                 split(col("dataframe_metric_cohort"), "_pct_sales_")[0],
                 "^(.*)$", "$1"
             )
        ).otherwise(
             # For Actual $: everything before the last underscore before pipe
             regexp_replace(
                 col("dataframe_metric_cohort"),
                 "^(.+)_([^_]+\\s*\\|.*)$", "$1"
             )
        )
    ).withColumn(
    ...
    other_cols = [c for c in df.columns if c != "dataframe_metric_cohort"]
    output_cols = ["dataframe_metric_cohort", "Metric", "cost format", "cohort"] + other_cols
